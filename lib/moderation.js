...
const CRITICAL_PATTERNS = [
  ...
  /\bcsam\b/i,
  /\bjailbait\b/i,
  // Solicitation patterns (e.g. "looking for cp", "trade cp", "share cp", "send corn")
  /\b(?:looking\s*for|want|need|trade|share|send|swap)\s+(?:cp|c\.p\.|csam|cheese\s*pizza|c[o0]rn|young\s*content)\b/i,
];

// ─── CATEGORY 2: Terrorism & Violent Extremism (CRITICAL — instant block) ───
const TERRORISM_PATTERNS = [
  /\b(?:join|support|pledge)\s+(?:isis|isil|daesh|al[\s-]*qaeda|aq(?:ap|im)|jnim|irgc)\b/i,
  /\bisis\b.*\b(?:soldier|fighter|brother|sister|martyr)\b/i,
  /\birgc\b/i,                                      // Islamic Revolutionary Guard Corps
...
const ELEVATED_PATTERNS = [
  ...
  { pattern: /\bkermit\b/i, label: 'self-harm-algospeak' },
  // Eating disorder promotion
  { pattern: /\bpro[\s-]*(?:ana|mia)\b/i, label: 'ed-promotion' },
  { pattern: /\b(?:thinspo|bonespo)\b/i, label: 'ed-promotion' },
  // Doxxing patterns
